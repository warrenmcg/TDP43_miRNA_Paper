...
if (@ARGV != 3)
{
	die "Usage: perl rename_allRNASeq_norm_files.pl <input> input_directory out_directory file_manifest.txt\n";
}
...
while($line=<FM>)
{
	...
	unless( $line =~ m"RNASeqV2	UNC	(.*?)	(.*?)	(.*?)	(.*?)	(.*?\.rsem.isoforms.normalized_results)") { next; }
	$barcode=$3;
	$sample=$5;
...
sub rename_file
{
	...
	if (exists $sample2barcode{$file})
	{
		...
		$new_file_name = "${out_dir}/${barcode}.rsem.isoforms.normalized_results";
